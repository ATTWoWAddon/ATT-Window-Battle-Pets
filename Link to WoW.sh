...
link_expansion() {
    if [[ -d "$1" ]]; then
        ...
        if [[ -d "$1/Interface/AddOns/ATT-Window-Battle-Pets" ]]; then
            echo "Removing ATT-Window-Battle-Pets in $1"
            rm -rf "$1/Interface/AddOns/ATT-Window-Battle-Pets"
        fi
        if [[ ! -d "$1/Interface/AddOns/ATT-Window-Battle-Pets" ]]; then
            echo "Adding ATT-Window-Battle-Pets in $1"
            mkdir -p "$1/Interface/AddOns"
            rsync -a  --link-dest="$PWD/" --exclude='.*' "$PWD/" "$1/Interface/AddOns/ATT-Window-Battle-Pets"
        fi
    fi
}
